# SiemensJava2026


# Solution:
I have a created a backend Springboot app that does the requiered busines logic. I have used Mailtrap to send the emails.
I have created a html for the view part. I use the Model-View-Controller pattern to show the data in view, controller to handle the interaction between view and model  and model to represent the data (there are also DTO for the JSON component communication). There is also a service layer that takes care of the data handling to leave the controller in a smalller size.

## Prerequisites
Install the needed dependencies/springboot
Used Java version 23

## Client Options

![Overview](photos/image.png)

![Routes & Trains](photos/image-1.png)


![Book Tickets](photos/image-2.png)

# Used Mail Trap for the email service
I needed a domain that I do not have to use a real email service. I do not have a device that runs non-stop to host my own dns server.
![Email Confirmation](photos/image-3.png)


## Admin Options
![Add stations menu](photos/image-4.png)


![Route Manager](photos/image-5.png)

![Train Viewer](photos/image-6.png)

![Booking Viewer](photos/image-7.png)

![Delay Manager](photos/image-11.png)

![Delay Effect](photos/image-9.png)
every passanger was notified of it
![Email Notifier](photos/image-10.png)

